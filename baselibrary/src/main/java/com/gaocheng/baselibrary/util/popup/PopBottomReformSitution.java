...
public class PopBottomReformSitution implements OnWheelChangedListener {


	private static PopBottomReformSitution cityUtils;

	public static PopBottomReformSitution getInstance() {
		if (cityUtils == null) {
			synchronized (PopBottomReformSitution.class) {
				if (cityUtils == null) {
					cityUtils = new PopBottomReformSitution();
				}
			}
		}
		return cityUtils;
	...
	public void showSelectDialog(final Context context , List<RainInfromUpdataLoadData.ReformSituationBean> mList, final onSelectCityFinishListener listener) {
...
//		cityDialog.onWindowAttributesChanged(wl);
	...
	private void setUpData(List<RainInfromUpdataLoadData.ReformSituationBean> mList) {
		List<String> list = new ArrayList<>();
		for (RainInfromUpdataLoadData.ReformSituationBean bean:mList){
			list.add(bean.Name);
		}
	...
	public interface onSelectCityFinishListener {
		public void onFinish(RainInfromUpdataLoadData.ReformSituationBean s);
	}
