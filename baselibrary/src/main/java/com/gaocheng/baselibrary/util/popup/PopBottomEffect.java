...
public class PopBottomEffect implements OnWheelChangedListener {


	private static PopBottomEffect cityUtils;

	public static PopBottomEffect getInstance() {
		if (cityUtils == null) {
			synchronized (PopBottomEffect.class) {
				if (cityUtils == null) {
					cityUtils = new PopBottomEffect();
				}
			}
		}
		return cityUtils;
	...
	public void showSelectDialog(final Context context , List<RainInfromUpdataLoadData.ReformEffectBean> mList, final onSelectCityFinishListener listener) {
...
//		cityDialog.onWindowAttributesChanged(wl);
	...
	private void setUpData(List<RainInfromUpdataLoadData.ReformEffectBean> mList) {
		List<String> list = new ArrayList<>();
		for (RainInfromUpdataLoadData.ReformEffectBean bean:mList){
			list.add(bean.Name);
		}
	...
	public interface onSelectCityFinishListener {
		void onFinish(RainInfromUpdataLoadData.ReformEffectBean s);
	}
