...
public class PopBottomUtil implements OnWheelChangedListener {


	private static PopBottomUtil cityUtils;

	public static PopBottomUtil getInstance() {
		if (cityUtils == null) {
			synchronized (PopBottomUtil.class) {
				if (cityUtils == null) {
					cityUtils = new PopBottomUtil();
				}
			}
		}
		return cityUtils;
	...
	public void showSelectDialog(final Context context ,List<String> mList, final onSelectCityFinishListener listener) {
...
//		cityDialog.onWindowAttributesChanged(wl);
	...
	private void setUpData(List<String> mList) {
		provinceAdapter = new CityWheelAdapter(context, mList);
