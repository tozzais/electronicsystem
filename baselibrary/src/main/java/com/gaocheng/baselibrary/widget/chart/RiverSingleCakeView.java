 ...
 * 河道 整治情况统计的 单饼图
 */
public class RiverSingleCakeView extends View {
    ...
    public RiverSingleCakeView(Context context) {
        this(context, null);
    }

    public RiverSingleCakeView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RiverSingleCakeView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                canvas.drawText(format.format(baseMessage.percent * 100 / total) + "%", textPointF.x, textPointF.y + (fm.descent - fm.ascent), textPaint);
    ...
    private void drawText(Canvas canvas) {
        for (int i = 0; i < textList.size(); i++) {
            String text = mCanvasList.get(i).contentAndNumber ;
